  ...
  useEffect(() => {
    async function loadProfile() {
      try {
        ...
        console.error("Failed to load footer profile", e);
      }
    }
    loadProfile();
  }, []);
      ...
      <div className="max-w-6xl mx-auto px-6 sm:px-10">
        ...
        <div className="flex flex-col lg:flex-row lg:items-start justify-between gap-8 pb-10 border-b border-gray-200">
          <div className="max-w-md">
            ...
            <p className="text-xs sm:text-sm font-medium text-gray-600 leading-relaxed">
              {profile.title}. Crafting user-centered interfaces, intuitive design systems, and engaging web experiences.
            </p>
          </div>
          ...
          <div className="flex items-center gap-3">
            ...
            {profile.socials?.behance && (
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                className="w-10 h-10 rounded-full bg-white border border-gray-200 text-[#0F1115] flex items-center justify-center hover:bg-blue-600 hover:text-white hover:border-blue-600 transition-all duration-200 shadow-sm font-black text-xs"
                aria-label="Behance"
              >
                Bē
              </a>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-8 py-10 text-xs sm:text-sm">
          ...
          <div>
            ...
            <div className="flex flex-col gap-2.5 font-semibold text-[#0F1115]">
              <a href={`mailto:${profile.email}`} className="hover:text-indigo-600 transition-colors flex items-center gap-1">
                <span className="truncate max-w-[170px]">{profile.email}</span>
        ...
        <div className="pt-8 border-t border-gray-200 flex flex-col sm:flex-row items-center justify-between text-xs font-semibold text-gray-500 gap-4">
          ...
          <div className="flex items-center gap-6">
            ...
            <a href={`mailto:${profile.email}`} className="hover:text-indigo-600 transition-colors">Email Me</a>
          </div>
        </div>
